...
for i,fichero in enumerate(ficheros):
	...
	for i  in range(0,11):
		...
		if i == 10:
			...
			x,y,w,h = cv2.boundingRect(contour_derecha[cuadro_derecha[2]])
			x = x + offsetx
			y = y + offsety
			w = w - offsetw
			h = h - offseth
			...
			#res = cv2.copyMakeBorder(res, 50,50,50,50,cv2.BORDER_CONSTANT)
			source = res # source is numpy array 
			...
			images_revistas.append(bitmap)

		else:
			x,y,w,h = cv2.boundingRect(contour_revistas[cuadro_revistas[10 + i]])
			x = x + offsetx
			y = y + offsety
			w = w - offsetw
			h = h - offseth
	...
	for i  in range(0,7):
		...
		offsetx = 20
		offsety = 10
		x,y,w,h = cv2.boundingRect(contour_albaran[cuadro_albaran[i]])
		x = x + offsetx
		y = y + offsety
		w = w - offsetw
		h = h - offseth
		...
		bitmap = cv.CreateImageHeader((source.shape[1], source.shape[0]), cv.IPL_DEPTH_8U, 3)
		cv.SetData(bitmap, source.tostring(), source.dtype.itemsize * 3 * source.shape[1])
		images_albaranes.append(bitmap)
